...
class MessageProcessor {
    ...
    validateGateway(gatewayUdi, gateway, callback) {
        let err = null;
        if (gateway == null) {
            err = new pip_services_commons_node_1.NotFoundException(this._correlationId, 'GATEWAY_NOT_FOUND', 'Gateway ' + gatewayUdi + ' was not found')
                .withDetails('gateway_udi', gatewayUdi);
        }
        if (gateway && !gateway.active) {
            err = new pip_services_commons_node_2.InvalidStateException(this._correlationId, 'GATEWAY_INACTIVE', 'Gateway ' + gateway.id + ' is not active')
    ...
    validateDevice(deviceUdi, device, callback) {
        let err = null;
        if (device == null) {
            err = new pip_services_commons_node_1.NotFoundException(this._correlationId, 'DEVICE_NOT_FOUND', 'Device ' + deviceUdi + ' was not found')
                .withDetails('device_udi', deviceUdi);
        }
        if (device && device.status != iqs_clients_devices_node_1.DeviceStatusV1.Active) {
            err = new pip_services_commons_node_2.InvalidStateException(this._correlationId, 'DEVICE_INACTIVE', 'Device ' + device.id + ' is not active')
